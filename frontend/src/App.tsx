...
import { clusterKinds, type ClusterResource, type ClusterSnapshot, type ResourceGraph } from '@simulator/shared/platform-contract';
...
export function App() {
  ...
  const [namespace, setNamespace] = useState('k8s-simulator');
  ...
  return (
    <main className="app-shell">
      <header>
        <div className="brand"><p className="eyebrow">PHASE 1 · READ-ONLY</p><h1>Kubernetes Cluster Explorer</h1></div>
        <div className="connection"><span className={error ? 'offline-dot' : 'online-dot'} /> {error ? error : 'LIVE'} <strong>{snapshot.resources.length} Objects</strong></div>
      </header>
